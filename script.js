let isPoweredOn = true; // Estado de la calculadora
let intentos = 0;
...
const sarcasmo = document.getElementById("sarcasmo");
function togglePower() {
    isPoweredOn = !isPoweredOn;
    if (!isPoweredOn) {
        display.value = ""; 
        sarcasmo.innerText = "Calculadora apagada";
        setTimeout(() => { sarcasmo.innerText = ""; }, 2000);
    } else {
        display.value = "";
    }
}
...
function appendToDisplay(value) {
    if (!isPoweredOn) return; // No permite escribir si está apagada
    if (value === "x") value = "*";
    if (value === "÷") value = "/";
    display.value += value;
}
...
// Resolver operación con acertijo
function solve() {
    if (!isPoweredOn) return;
    ...
    try {
        ...
        if (respuestaUsuario && respuestaUsuario.toLowerCase().trim() === acertijoSeleccionado.respuesta) {
            display.value = convertirAChino(resultado);
            intentos = 0;
        } else {
            intentos++;
            mostrarSarcasmo();
...
// Convertir resultado a números chinos
function convertirAChino(numero) {
    let numerosChinos = { "0": "零", "1": "一", "2": "二", "3": "三", "4": "四", "5": "五", "6": "六", "7": "七", "8": "八", "9": "九" };
    return numero.toString().split('').map(d => numerosChinos[d] || d).join('');
}
